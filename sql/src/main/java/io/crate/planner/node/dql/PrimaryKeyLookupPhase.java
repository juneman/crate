...
public class PrimaryKeyLookupPhase extends AbstractProjectionsPhase implements CollectPhase {
    ...
    public HashMap<Integer, List<DocKeys.DocKey>> docKeysByShard(ClusterService clusterService) {
        HashMap<Integer, List<DocKeys.DocKey>> result = new HashMap();
        for (DocKeys.DocKey docKey: docKeys()) {
            ShardId shardId = shardId(clusterService, tableIdent().indexName(), docKey.id(), docKey.routing());
            Integer id = shardId.getId();
            ArrayList<DocKeys.DocKey> docKeyList = new ArrayList<>();
            if (result.get(id) != null) {
                docKeyList = (ArrayList<DocKeys.DocKey>) result.get(id);
            }
            docKeyList.add(docKey);
            result.put(id, docKeyList);
        }
        return result;
    }

    @Nullable
    private ShardId shardId(ClusterService clusterService, String indexName, String id, @Nullable String routing) {
